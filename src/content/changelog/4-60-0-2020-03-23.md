...
- Previously, the `name` field for Create Domain Record ([POST /domains/{domainId}/records](/api/v4/domains-domain-id-records/#post)) and Update Domain Record ([PUT /domains/{domainId}/records/{recordId}](/api/v4/domains-domain-id-records-record-id/#put)) did not accept wildcards for Domain Records of type `TXT` . This has been fixed and the field now accepts wildcards.