...
* Fixed an issue that allowed Linodes with IPv6 /116 pools to attempt to migrate to NGN data centers like Toronto and Mumbai. IPv6 /116 pools are not supported by NGN data centers, and so these migrations would never actually progress. A Linode with IPv6 /116 pools will now be prevented from initiating a migration to an NGN data center.