(function($) {
  ...
  Search = {
    ...
    handler: function(response) {
      ...
      if (Search.repsLeft === 1 && response.queries.nextPage) {
      ...
      if (Search.repsLeft === Search.numReps && response.queries.previousPage) {
      ...
      if (response.queries.nextPage) {
        Search.repsLeft--;
        if (Search.repsLeft > 0) Search.run();
      }
    },
    init: function() {
    ...
    run: function() {
      var elem = document.createElement('script'),
          page = Search.getPageNumber() - 1,
          rep = Search.numReps - Search.repsLeft,
          filter = 'more:pagemap:techarticle',
          args = ['key=AIzaSyCMGfdDaSfjqv5zYoS0mTJnOT3e9MURWkU',
                  'cx=000157629893119917628:5rcen_5ss4c',
                  'callback=Search.handler',
                  'q=' + Search.getQuery() + ' ' + filter,
                  'start=' + (((page * Search.numReps + rep) * Search.resultsPerPage) + 1),
